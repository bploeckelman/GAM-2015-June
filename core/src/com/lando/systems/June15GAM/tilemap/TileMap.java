...
public class TileMap {
    ...
    public TileMap(int xTiles, int yTiles) {
        ...
        tiles = new TileType[yTiles][xTiles];
        for (int y = 0; y < tiles.length; ++y) {
            for (int x = 0; x < tiles[y].length; ++x) {
                int r = MathUtils.random(1, 100);
                if      (r >= 50) tiles[y][x] = TileType.GROUND_GRASS;
                else if (r >= 40) tiles[y][x] = TileType.GROUND_CLAY;
                else if (r >= 30) tiles[y][x] = TileType.GROUND_SAND;
                else if (r >= 20) tiles[y][x] = TileType.GROUND_WATER;
                else              tiles[y][x] = TileType.GROUND_CONCRETE;
            }
        }
    }
